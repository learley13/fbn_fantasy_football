...
export function AnalyticsHub({ reports }: { reports: AnalyticsReport[] }) {
  ...
  return <>
    ...
    <section className="analytics-cards" aria-label="League highlights"><article><span>Scoring leader</span><strong>{team(mostPoints, "analytics-team-link")}</strong><b>{decimals.format(mostPoints?.pf || 0)} PF</b></article><article className="positive"><span>Luckiest record</span><strong>{team(luckiest, "analytics-team-link")}</strong><b>+{decimals.format(luckiest?.luck || 0)} wins</b></article><article className="negative"><span>Most unlucky</span><strong>{team(unluckiest, "analytics-team-link")}</strong><b>{decimals.format(unluckiest?.luck || 0)} wins</b></article><article><span>Weekly high</span><strong>{team(highScorer, "analytics-team-link")}</strong><b>{decimals.format(weeklyHigh?.points || 0)} points</b></article></section>
